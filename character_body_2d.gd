...
func get_relative_direction(movement_angle: float) -> RelativeDirection:
	...
	if abs(relative_movement_vector.x) > abs(relative_movement_vector.y):
		return RelativeDirection.ADVANCING if relative_movement_vector.x > 0 else RelativeDirection.RETREATING
	else:
		return RelativeDirection.SIDESTEPPING_CW if relative_movement_vector.y > 0 else RelativeDirection.SIDESTEPPING_CCW
	...
	if roll_timer.is_stopped():
		...
		match roll_type:
			RelativeDirection.HOLDING_POSITION:
				transform = transform.orthonormalized() * Transform2D(Vector2(-1 * cos(2 * TAU * roll_timer.time_left), 0), Vector2(0, -1 * cos(2 * TAU * roll_timer.time_left)), Transform2D.IDENTITY.origin)
			RelativeDirection.ADVANCING, RelativeDirection.RETREATING:
				transform = transform.orthonormalized() * Transform2D(Vector2(-1 * cos(2 * TAU * roll_timer.time_left), 0), Transform2D.IDENTITY.y, Transform2D.IDENTITY.origin)
